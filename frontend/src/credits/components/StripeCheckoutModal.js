...
const StripeCheckoutModal = ({ show, onHide, onSuccess, currentUsage }) => {
  ...
  const ratePerCredit = thirdTier ? parseFloat(thirdTier.price) / thirdTier.extra_amount : 0.2999;

  useEffect(() => {
    if (show) {
      CreditsService.getCreditsTiers().then((data) => {
  ...
  const handlePurchase = async () => {
    ...
    setLoading(true);
    try {
      const response = await CreditsService.purchaseExtraCalls(selectedTier, isCustomTierSelected ? parsedCustomQty : undefined);
      if (response?.checkout_url) {
        window.location.href = response.checkout_url;
      } else {
        // Previously failures were silent; surface the specific reason.
        toast.error(t(response?.error || 'payment-failed', { ns: 'billing' }));
      }
    } catch (error) {
      toast.error(t('payment-failed', { ns: 'billing' }));
    } finally {
      setLoading(false);
    }
  };
  ...
  const calculateCustomPrice = () => {
    ...
    return (parsedCustomQty * ratePerCredit).toFixed(2);
  };
  ...
  return (
    <Modal show={show} onHide={onHide} centered className="migratis-modal">
      <Modal.Header className="migratis-modal-header">
        ...
        <Modal.Title className="migratis-modal-title">{t('purchase-extra-credits')}</Modal.Title>
      ...
      <Modal.Body className="migratis-modal-body">
        ...
        <div className="text-center mb-4">
          ...
          >
            {t('credits', { remaining: currentUsage?.credits || 0 })}
          </Badge>
        </div>
              ...
              >
                <div className="d-flex justify-content-between align-items-center">
                  <div>
                    {!isLastTier && (
                      <>
                        <strong>{tier.extra_amount} {t('extra-credits')}</strong>
                        <div className="text-muted small">
                          {t('price-per-credit', { price: (tier.price / tier.extra_amount).toFixed(2) })}
                        </div>
                      </>
                    )}
                    {isLastTier && (
                      <>
                        <InputGroup onClick={(e) => e.stopPropagation()}>
                          ...
                          />
                          <InputGroup.Text style={{ fontWeight: 'bold' }}>{t('extra-credits')}</InputGroup.Text>
                        </InputGroup>
                        <div className="text-muted small mt-1">
                          {customQtyValid
                            ? t('price-per-credit', { price: ratePerCredit.toFixed(2) })
